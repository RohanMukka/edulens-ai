  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="md:col-span-2 border-border/50 shimmer">
...
export function GraphSkeleton() {
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <div className="h-16 border-b border-border/60 flex items-center px-6 gap-4">
        <div className="w-8 h-8 bg-muted rounded-md" />
        <div className="w-48 h-6 bg-muted rounded" />
      </div>
      <div className="flex-1 relative overflow-hidden">
        <div className="absolute inset-0 grid grid-cols-4 grid-rows-3 gap-12 p-12 opacity-20">
          {[...Array(12)].map((_, i) => (
            <div key={i} className="w-24 h-24 rounded-full bg-muted animate-pulse" />
          ))}
        </div>
      </div>
    </div>
